...
class Twitter extends tmhOAuth {
    ...
    public function __construct(Config $config, SessionStore $session) {
        if ($config->has('twitter::config')) {
            $this->tconfig = $config->get('twitter::config');
        } else if ($config->get('twitter')) {
            $this->tconfig = $config->get('twitter');
        } else {
     ...
     * parametros requeridos: user_id ou screen_name
     */
